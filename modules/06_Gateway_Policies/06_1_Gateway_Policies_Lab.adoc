...
This is a standard policy plug-in available in 3scale API Management that can be configured directly in the service configuration. This policy is useful when the service is configured to use *OpenId Connect*
...
When an application is created for a 3scale service, a client corresponding to it is created in RH-SSO. 
...
Create two users (normal,admin) in threescale_realm for testing the policy
...
* Admin user will return a response with http status code as 200
* Admin user will return a response with http status code as 403. Response will be *Not authorized to use the operation*
